# Materi Pembelajaran: JavaScript → TypeScript → Express → REST API

## 📚 Deskripsi Kursus

Kursus ini dirancang untuk pemula yang ingin memahami cara membuat REST API menggunakan Express.js dan TypeScript. Materi disusun secara bertahap mulai dari dasar JavaScript, kemudian TypeScript, lalu Express.js, dan akhirnya membangun REST API yang lengkap.

## 🎯 Tujuan Pembelajaran

Setelah menyelesaikan kursus ini, peserta diharapkan dapat:
- Memahami konsep dasar JavaScript modern (ES6+)
- Menggunakan TypeScript untuk type safety
- Membangun REST API dengan Express.js
- Memahami struktur project Express yang baik
- Menggunakan Git dan GitHub untuk version control
- Testing API dengan Postman dan automated testing
- Menerapkan best practices dalam pengembangan backend

## 📖 Struktur Materi

### [Modul 1: JavaScript Dasar](./modul-01-javascript-dasar/README.md)
**Durasi: 2-3 sesi**

- Variabel dan Tipe Data
- Fungsi dan Arrow Functions
- Array Methods
- Object dan Destructuring
- Async/Await dan Promises
- Modules (import/export)

### [Modul 2: Pengenalan TypeScript](./modul-02-typescript/README.md)
**Durasi: 2-3 sesi**

- Mengapa TypeScript?
- Types dan Interfaces
- Generics
- Type Inference
- Migrasi dari JavaScript ke TypeScript

### [Modul 3: Git dan GitHub untuk Pemula](./modul-03-git-github/README.md)
**Durasi: 2-3 sesi**

- Pengenalan Git dan Version Control
- Git Commands Dasar
- Branch dan Merge
- GitHub (Remote Repository)
- Menggunakan Sourcetree (GUI Git)
- Best Practices dan Workflow

**Catatan:** Modul Git sebaiknya dipelajari sebelum mulai project besar agar bisa menggunakan version control sejak awal.

### [Modul 4: Express.js Dasar](./modul-04-express-dasar/README.md)
**Durasi: 2-3 sesi**

- Pengenalan Express.js
- Routing dan HTTP Methods
- Middleware
- Request dan Response Objects
- Error Handling

### [Modul 5: REST API Development](./modul-05-rest-api/README.md)
**Durasi: 3-4 sesi**

- Konsep REST API
- CRUD Operations
- Validation dan Error Handling
- Authentication dengan JWT
- Database Integration (Sequelize)

### [Modul 6: Testing API](./modul-06-testing-api/README.md)
**Durasi: 2-3 sesi**

- Pengenalan Testing API
- Testing dengan Postman (Manual Testing)
- Unit Testing dengan Jest
- Integration Testing
- Test Coverage dan Best Practices
- Advanced Testing Techniques

**Catatan:** Modul Testing dipelajari setelah REST API agar peserta bisa langsung test API yang sudah dibuat.

### [Modul 7: Praktik dengan Boilerplate](./modul-07-praktik-boilerplate/README.md)
**Durasi: 2-3 sesi**

- Setup Project
- Struktur Project
- Implementasi Fitur Baru
- Best Practices

**Catatan:** Modul ini adalah praktik akhir dimana peserta akan menggunakan pengetahuan testing yang sudah dipelajari untuk memastikan kode yang dibuat berkualitas.

## 🚀 Cara Menggunakan Materi Ini

1. **Untuk Instruktur:**
   - Baca setiap modul sebelum mengajar
   - Siapkan contoh kode yang akan didemonstrasikan
   - Siapkan latihan untuk setiap modul
   - Gunakan repository boilerplate sebagai referensi akhir

2. **Untuk Peserta:**
   - Ikuti urutan modul secara berurutan
   - Praktikkan setiap konsep yang dipelajari
   - Kerjakan latihan di setiap modul
   - Jangan terburu-buru, pahami konsepnya terlebih dahulu

## 📁 Repository Referensi

Repository yang digunakan sebagai referensi:
- [Boilerplate Express TypeScript](https://github.com/teramuza/boilerplate-express-ts)

## 💡 Tips Mengajar

1. **Mulai dengan JavaScript**: Jangan langsung ke TypeScript, biarkan peserta memahami JavaScript dulu
2. **Transisi Bertahap**: Setelah peserta nyaman dengan JavaScript, baru perkenalkan TypeScript
3. **Praktik Langsung**: Setiap konsep harus langsung dipraktikkan dengan kode
4. **Gunakan Contoh Nyata**: Berikan contoh yang relevan dengan REST API
5. **Review Berkala**: Lakukan review materi sebelumnya sebelum masuk ke materi baru

## 📝 Catatan Penting

- Materi ini dirancang untuk pemula, jadi penjelasan dibuat sesederhana mungkin
- Setiap modul memiliki latihan yang harus dikerjakan
- Progress bertahap sangat penting - jangan skip modul
- Repository boilerplate adalah target akhir yang ingin dicapai
